...
<div class="col-xs-12 col-lg-12">
    ...
    <div class="<?php echo $model->isNewRecord ? 'box-success' : 'box-info'; ?> box col-xs-12 col-lg-12 no-padding">
        <div class="box-header with-border">
            <h3 class="box-title"><i class="fa fa-user"></i><sub><i class="fa fa-info-circle"></i></sub> Student Details
            </h3>
        </div>
    ...
    <div class="<?php echo $model->isNewRecord ? 'box-success' : 'box-info'; ?> box col-xs-12 col-lg-12 no-padding">
        <div class="box-header with-border">
            <h3 class="box-title"><i class="fa fa-inr"></i><sub><i class="fa fa-info-circle"></i></sub> Fees Collection
                Category : <?php echo $FccModel->fees_collect_name; ?></h3>
        </div>
        <div class="box-body table-responsive">
            ...
            foreach ($feesDetails as $key => $value) {
                ...
                echo '</tr>';
                $totalAmount += is_numeric($value['fees_details_amount']) ? $value['fees_details_amount'] : 0;
            }
            $totalPay = $model->getStuTotalPayFees($stuData->stu_master_id, $FccModel->fees_collect_category_id);
            ...
            echo '<tr><th colspan=2 class="text-right">Total Paid Fees</th><td>' . $totalPay . '</th></tr>';
        ...
        <div class="box-footer">
            <div class="pull-right" style="padding-bottom:10px">
                <?php if ($collectOn) {
                    echo Html::submitButton($model->isNewRecord ? '<i class="fa fa-plus-circle"></i> Take Fees' : '<i class="fa fa-pencil-square-o"></i> Update Fees', ['class' => $model->isNewRecord ? 'btn btn-primary' : 'btn btn-info']);
                } ?>

                <?php if ($printOn & $model->isNewRecord) {
                    echo Html::a('<i class="fa fa-print"></i> Print receipt', ['print-common-receipt', 'sid' => $stuData->stu_master_id, 'fcid' => $FccModel->fees_collect_category_id], ['class' => 'btn btn-warning', 'target' => '_blank']);
                } ?>

                <?php if (!$model->isNewRecord) {
                    echo Html::a('Cancel', ['pay-fees', 'sid' => $stuData->stu_master_id, 'fcid' => $FccModel->fees_collect_category_id], ['class' => 'btn btn-default']);
                } ?>
            </div>
        </div>
    </div><!---End box div---->
